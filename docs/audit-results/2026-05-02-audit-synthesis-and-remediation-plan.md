...
- Remaining customer visual polish and frontend warning curation should be separate backlog packets unless the release definition expands to include them. Staff table/card mobile optimization, the failed-refund admin exception queue, the customer notification feed, and the backend PyJWT warning/status curation slice have been handled in follow-up backlog packets.
