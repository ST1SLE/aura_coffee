...
Target duration: ~90 minutes end-to-end, with all mocks green. Keep a four-pane terminal layout (logs) + a browser window for each SPA + a fifth terminal for curl.
...
| Menu | 1 category, 1 drink "Капучино (QA)" 600 ₽ + size M |
...
export CUST_TOKEN=$(curl -s -X POST $BASE/api/v1/auth/verify-code \
  -H 'Content-Type: application/json' \
...
**UI.** `/menu` (customer SPA home). Categories render as tabs, items as cards. Click "Капучино (QA)" → drawer opens with size options (M, 600 ₽) and any modifiers.
...
**UI.** `/admin/menu` (both admin and barista can do this — phase 5 menu RBAC) → click the availability toggle on "Капучино (QA)". Customer SPA menu should stop showing the drink within a few seconds (menu is cacheless on the read path; confirm via `curl $BASE/api/v1/menu | jq '.categories[0].items[0].is_available'`).
...
curl -s -X PATCH $BASE/api/v1/admin/menu/items/1/availability \
  -H "Authorization: Bearer $BARISTA_TOKEN" -H 'Content-Type: application/json' \
  -d '{"is_available":false}' | jq .is_available
# Expect: false
# Flip back:
curl -s -X PATCH $BASE/api/v1/admin/menu/items/1/availability \
  -H "Authorization: Bearer $BARISTA_TOKEN" -H 'Content-Type: application/json' \
  -d '{"is_available":true}' | jq .is_available
```
