...
const mockCategories = [{ id: 1, type: 'drink' as const, name_ru: 'Кофе', name_en: 'Coffee', sort_order: 0, is_visible: true }];
...
describe('MenuItemFormDialog', () => {
  ...
  test('MenuItemFormDialog has name_ru, name_en, description_ru, description_en, sort_order, image_url', () => {
    render(
      <MenuItemFormDialog
        open={true}
        onClose={vi.fn()}
  ...
  test('MenuItemFormDialog submits bilingual payload with base_price', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Name (RU)'), { target: { value: 'Латте' } });
    fireEvent.change(screen.getByLabelText('Name (EN)'), { target: { value: 'Latte' } });
    fireEvent.change(screen.getByLabelText('Price (₽)'), { target: { value: '350' } });
