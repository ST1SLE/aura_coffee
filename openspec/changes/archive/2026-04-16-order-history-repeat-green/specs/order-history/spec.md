...
_References: PDD §7.7 (Order history, Repeat Order Chain), §5.2 (Order, OrderItem), INV-002 (auth for reads that expose user-scoped data), INV-013 (PII isolation — reads by opaque UUID), INV-014 (order_items are immutable snapshots — the service only reads them)._

_Previously: the `order-history` capability was introduced by the RED change `2026-04-16-order-history-repeat-red` with "module/route MUST NOT exist" scenarios. RED was not synced to the living specs by design (RED scenarios contradict the forward-going contract). This GREEN change registers the live contract in full._

_Now: `core_api.services.order_history.list_orders`, the five `core_api.schemas.order_history` DTOs, `core_api.routers.order_history` with `GET /api/v1/orders`, and the `ROUTE_MATRIX` CUSTOMER entry for that route all exist. All RED tests targeting this capability pass._
