...
- `/promos` — Promocode management (protected, `allowedRoles: ['admin', 'barista']`) with an **inner** `ProtectedRoute allowedRoles={['admin']}` wrapper — admin-only.
- `/settings` — Shop settings (protected, `allowedRoles: ['admin', 'barista']`) with an **inner** `ProtectedRoute allowedRoles={['admin']}` wrapper — admin-only.
...
The admin/barista routes (`/`, `/orders`, `/menu`, `/users`, `/promos`, `/settings`) SHALL be grouped under a single `ProtectedRoute` wrapper with `allowedRoles={['admin', 'barista']}` around the shared `Layout` element. The `/promos` and `/settings` routes SHALL be additionally wrapped, inside that shared Layout, by an inner `ProtectedRoute allowedRoles={['admin']}` that redirects non-admin staff before the page renders (double-guard pattern: outer role-group gate + inner capability gate enforces INV-010 without relying on sidebar filtering alone). The `/courier` route SHALL be wrapped, separately, by a `ProtectedRoute` with `allowedRoles={['admin', 'courier']}` around a lightweight shell (`CourierShell`) that renders only the `LanguageSwitcher` and `NotificationList`, **not** the admin `Layout`. `ProtectedRoute` SHALL redirect unauthenticated users to `/login?returnUrl=<current path>` (see spec `admin-auth-ui` §Admin ProtectedRoute guard). The `/login` route SHALL NOT be wrapped — it must be reachable without a token.

**Previously:** The `/settings` route was wrapped only by the shared admin+barista `ProtectedRoute`. A barista entering `/admin/settings` in the URL bar would reach the SettingsPage stub (client-only guard was sidebar filtering).

**Now:** `/settings` is additionally wrapped by an inner `ProtectedRoute allowedRoles={['admin']}`, matching the existing `/promos` pattern. A barista attempting to enter `/admin/settings` is redirected to `/admin/` (the role-based redirect from `ProtectedRoute`); the SettingsPage is never mounted. Server-side RBAC on `GET/PUT /api/v1/admin/settings` remains authoritative per INV-010.

Refs: spec `admin-auth-ui`; spec `staff-auth`; spec `courier-panel-ui`; spec `shop-settings-admin-api`; PDD §4.5, §7.1 Phase 6 item 3; INV-010.
...
#### Scenario: Barista redirected away from /settings (inner guard)
- **WHEN** an authenticated barista navigates to `/admin/settings`
- **THEN** they are redirected to `/admin/` (role-based redirect from the inner `ProtectedRoute allowedRoles={['admin']}`), and `SettingsPage` is NOT mounted

#### Scenario: Admin reaches /settings
- **WHEN** an authenticated admin navigates to `/admin/settings`
- **THEN** the `SettingsPage` renders inside the shared `Layout`
