...
_References: PDD §3 (Domain Language — Customer, Cart, Order, Order Item, Payment, Loyalty Transaction, Promocode), PDD §5 (Database Schema — orders, order_items, payments, loyalty_transactions, promocode_usages), PDD §6.1 (Order Lifecycle State Machine), PDD §7.1 Phase 3 items 1-2 (Checkout service and routes), PDD §7.2 (Order Pricing Chain), PDD §7.4 (Delivery fee), PDD §7.7 (Menu archival vs order history), INV-001 (prepayment only), INV-002 (auth for mutations), INV-004 (atomic financial transactions), INV-006 (stop list), INV-010 (role isolation), INV-013 (PII isolation / 152-FZ), INV-014 (immutable order items), INV-016 (exhaustive state machines)._

_This GREEN delta lands the implementation for requirements sealed by `order-checkout-red`. The requirement and scenario set is unchanged from RED — duplicating the ADDED block here is the project's convention for tying implementation to its contract at archive time and matches the `add-public-menu-green` precedent._
