...
_References: PDD §4.5 (Admin panel surfaces), §5.4 (partial index `orders (status) WHERE status NOT IN (COMPLETED, CANCELLED)`), §6.1 (order state machine), INV-010 (role isolation — staff queries live behind staff endpoints), INV-013 (PII isolation — `user_id` is an opaque UUID)._
...
- Ordering:
  - When `status_filter == "active"` OR `status_filter` is a non-finalized `OrderStatus` (i.e. any value other than `COMPLETED` or `CANCELLED`) — rows SHALL be ordered `created_at DESC`.
...
In the RED change this symbol MUST NOT exist. Every scenario below MUST fail with `ImportError` / `ModuleNotFoundError` when the test imports the target symbol inside the test body, or MUST fail because the GREEN-phase contract is not yet implemented.

#### Scenario: RED — staff list symbol is absent
- **WHEN** a test body executes `from core_api.services.order_history import list_orders_for_staff`
...
The system SHALL expose a service function `get_order_for_staff(order_id, db_session) -> OrderResponse` at `core_api.services.order_history`. The function SHALL NOT perform an ownership check and SHALL return the order for any `user_id` that owns it. If the id is unknown the function SHALL raise a designated "order not found" domain error.

In the RED change this symbol MUST NOT exist.

#### Scenario: RED — staff detail symbol is absent
- **WHEN** a test body executes `from core_api.services.order_history import get_order_for_staff`
- **THEN** the import SHALL raise `ImportError` / `ModuleNotFoundError`, failing the test
...
- **THEN** the function SHALL raise a designated "order not found" domain error

### Requirement: GET /api/v1/admin/orders list endpoint

The system SHALL expose `GET /api/v1/admin/orders` at `core_api.routers.admin_orders`. The route SHALL be open to `{ADMIN, BARISTA}` only via RBAC; CUSTOMER and COURIER SHALL receive 403, and requests without a valid Bearer token SHALL receive 401. Query parameters:
...
In the RED change this route MUST NOT be registered. Requests SHALL fail with the RBAC middleware's default-deny (403) or with 404 depending on middleware ordering.

#### Scenario: RED — route is not registered
- **WHEN** a test inspects `app.routes` for a GET matching `/api/v1/admin/orders`
- **THEN** the match count SHALL equal 0 (route is absent until GREEN)
...
- **GIVEN** the route is registered and orders exist across active and finalized statuses for multiple users
...
- **GIVEN** the route is registered and active orders exist with both `type=PICKUP` and `type=DELIVERY`
...
- **GIVEN** the route is registered and orders are seeded
...
In the RED change this route MUST NOT be registered.

#### Scenario: RED — route is not registered
- **WHEN** a test inspects `app.routes` for a GET matching `/api/v1/admin/orders/{order_id}`
- **THEN** the match count SHALL equal 0
...
- **GIVEN** the route is registered and an order belongs to user B
...
In the RED change these rows MUST NOT exist.

#### Scenario: RED — rbac matrix is missing rows
- **WHEN** a test asserts `("GET", "/api/v1/admin/orders") in ROUTE_MATRIX`
- **THEN** the assertion SHALL fail

#### Scenario: GREEN contract — admin-orders routes map to admin + barista only
...
#### Scenario: GREEN contract — admin-orders routes are not public
- **GIVEN** GREEN has landed
