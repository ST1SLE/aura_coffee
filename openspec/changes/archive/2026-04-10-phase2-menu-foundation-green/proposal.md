> **TDD phase: GREEN.** This is the second of a two-change pair (`phase2-menu-foundation-red` → `phase2-menu-foundation-green`). This change lands the implementation that makes the RED change's failing tests pass, plus the migration and verification steps. Both changes share identical proposal, design, and specs — only `tasks.md` differs.
