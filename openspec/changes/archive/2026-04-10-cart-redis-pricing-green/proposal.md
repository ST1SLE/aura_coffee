> **TDD phase: GREEN.** This is the second of a two-change pair (`cart-redis-pricing-red` → `cart-redis-pricing-green`). The RED change landed PREREQ setup and failing tests. This change implements the behavior that turns those tests green, then refactors. Both changes share identical proposal, design, and specs — only `tasks.md` differs.
