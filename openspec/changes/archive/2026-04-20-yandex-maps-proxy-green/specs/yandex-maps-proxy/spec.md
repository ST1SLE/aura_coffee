...
<!-- Source: PDD §7.3 (Address Validation Chain), §8.3 (Yandex.Maps), §8.4 (Secrets table), INV-015 (secrets out of code).
     GREEN re-applies the exact ADDED delta from `yandex-maps-proxy-red`. On archive of GREEN, the capability
     is promoted to `openspec/specs/yandex-maps-proxy/spec.md`. No requirement text diverges from RED. -->
