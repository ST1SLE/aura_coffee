...
_References: PDD §4.5 (ADMIN-only аналитика), §6.1 (COMPLETED terminal), §7.1 Phase 6 item 1, INV-010 (role isolation), INV-014 (snapshot fidelity). All five requirements below were seeded in the RED archive with RED-only "symbol absent / route not registered / matrix row missing" clauses. GREEN removes those clauses so each requirement now describes the actual runtime behavior._
