...
_References: PDD §4.5 (admin panel staff feed), §6.1 (order state machine — staff transitions), §7.1 Phase 6 (admin panel orphan fix), INV-010 (role isolation), INV-013 (PII isolation — staff UI renders only fields exposed by `order_history.OrderResponse`), INV-016 (state-machine exhaustiveness — UI hides illegal buttons, server is the authority)._
