> **TDD phase: GREEN.** This is the second of a two-change pair (`menu-admin-crud-red` → `menu-admin-crud-green`). This change lands the implementation that makes the RED change's failing tests pass, followed by REFACTOR and VERIFY. Both changes share identical proposal, design, and specs — only `tasks.md` differs.
