...
The customer SPA SHALL expose a typed loyalty API client module at `web/customer/src/api/loyalty.ts` that wraps the backend loyalty endpoints, uses the shared `authenticatedFetch` client, and runtime-asserts response shape before returning typed data.
...
- **WHEN** the backend returns a payload that does not match the expected shape
- **THEN** the module throws a `LoyaltyApiError` rather than returning partial or untyped data
...
`LoyaltyPage` SHALL render a paginated list of the customer's loyalty transactions using per-page accumulation (`useState` + `useEffect`) and display `TransactionRow` for each item.
