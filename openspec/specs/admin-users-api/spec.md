...
#### Scenario: RED — admin-users list service symbol is absent
- **WHEN** a test body executes `from core_api.services.admin_users import list_users`
...
#### Scenario: RED — admin-users detail service symbol is absent
- **WHEN** a test body executes `from core_api.services.admin_users import get_user_detail`
- **THEN** the import SHALL raise `ImportError` / `ModuleNotFoundError`, failing the test
...
#### Scenario: RED — block_user service symbol is absent
- **WHEN** a test body executes `from core_api.services.admin_users import block_user`
...
#### Scenario: RED — unblock_user service symbol is absent
- **WHEN** a test body executes `from core_api.services.admin_users import unblock_user`
- **THEN** the import SHALL raise `ImportError` / `ModuleNotFoundError`
...
#### Scenario: RED — adjust_loyalty service symbol is absent
- **WHEN** a test body executes `from core_api.services.admin_users import adjust_loyalty`
- **THEN** the import SHALL raise `ImportError` / `ModuleNotFoundError`
...
- **WHEN** a test asserts `("GET", "/api/v1/admin/users") in ROUTE_MATRIX`
- **THEN** the assertion SHALL fail

#### Scenario: GREEN contract — admin-users list route maps to ADMIN only
- **GIVEN** GREEN has landed
- **WHEN** a test reads `ROUTE_MATRIX[("GET", "/api/v1/admin/users")]`
- **THEN** the value SHALL equal `{ADMIN}`

#### Scenario: GREEN contract — admin-users detail route maps to ADMIN only
- **WHEN** a test reads `ROUTE_MATRIX[("GET", "/api/v1/admin/users/{user_id}")]`
- **THEN** the value SHALL equal `{ADMIN}`

#### Scenario: GREEN contract — admin-users block route maps to ADMIN only
- **WHEN** a test reads `ROUTE_MATRIX[("POST", "/api/v1/admin/users/{user_id}/block")]`
- **THEN** the value SHALL equal `{ADMIN}`

#### Scenario: GREEN contract — admin-users unblock route maps to ADMIN only
- **WHEN** a test reads `ROUTE_MATRIX[("POST", "/api/v1/admin/users/{user_id}/unblock")]`
- **THEN** the value SHALL equal `{ADMIN}`

#### Scenario: GREEN contract — admin-users loyalty/adjust route maps to ADMIN only
- **WHEN** a test reads `ROUTE_MATRIX[("POST", "/api/v1/admin/users/{user_id}/loyalty/adjust")]`
- **THEN** the value SHALL equal `{ADMIN}`

#### Scenario: GREEN contract — admin-users routes are not public
