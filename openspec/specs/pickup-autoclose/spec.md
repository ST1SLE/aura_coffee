# pickup-autoclose Specification

## Purpose
Auto-close stale pickup/READY orders after `shop_settings.auto_close_minutes` via a Celery Beat scheduler → worker pipeline. Reuses `order_lifecycle` as the single entry-point for state transitions (INV-016) and fires loyalty accrual as a side-effect (INV-003). No SMS is sent for auto-closure per PDD §6.1.
