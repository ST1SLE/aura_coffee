...
The system SHALL register the new `/api/v1/admin/menu/...` routes in `core_api.rbac_matrix.ROUTE_MATRIX`. Mutating routes on categories, items, modifiers, and sizes SHALL map to `{admin}`. The two `PATCH .../availability` routes SHALL map to `{admin, barista}`. The admin read endpoints (`GET /categories`, `GET /items`, `GET /items/{id}`, `GET /modifiers`) SHALL map to `{admin, barista}`. The `PUT /api/v1/admin/menu/items/{item_id}/modifiers` route SHALL be registered in `ROUTE_MATRIX` with role set `{"admin"}`. No admin menu route SHALL appear in `PUBLIC_ROUTES`.
